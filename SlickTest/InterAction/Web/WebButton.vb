...
''' <summary>
''' The WebButton must be a input with the type button, 
''' reset, and submit.
''' </summary>
''' <remarks></remarks>
...
#Region "Constructor"
    ...
    Protected Friend Overrides Sub Init()
        HtmlTags = APIControls.WebTagDefinitions.ButtonTags
    End Sub
#End Region

    Private Function GetButtonAPI() As WebButtonAPI
        Return New WebButtonAPI(Me.CurrentElement)
    End Function

    ''' <summary>
    ''' Returns the type of button based upon the input type.
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function GetButtonType() As String
        ExistsWithException()
        Return GetButtonAPI().ButtonType()
    End Function
End Class
